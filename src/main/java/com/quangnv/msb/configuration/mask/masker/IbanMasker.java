...
/**
 * Masker that masks a potential IBAN while still providing enough information to have it easily identifiable for log analysis. The first three and last 4 characters are preserved, while the others are replaced with the mask character.
 *
 * A potential IBAN is considered any sequence of characters that starts with the country code, has the third character a digit and has the right length. For performance/speed considerations, additional checks are <strong>NOT</strong> made.
 *
 * Ex: GT20AGRO00000000001234567890 will become GT2*********************7890
 */
public class IbanMasker implements LogMasker {
    ...
    private boolean shouldMask(StringBuilder builder, int buffLength, int start, Integer ibanLength) {
        ...
        if (buffLength == start + ibanLength || indexOfNextDelimiter(builder, start, buffLength) == start + ibanLength) {
            return true;
        }
